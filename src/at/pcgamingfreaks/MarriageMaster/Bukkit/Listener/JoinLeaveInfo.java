...
public class JoinLeaveInfo implements Listener
{
	...
	public void onJoin(MarriedPlayerJoinEvent event)
	{
		...
		for(MarriagePlayer partner : player.getPartners())
		{
			if(partner.isOnline())
			{
				messageNowOnline.send(partner.getPlayer().getPlayer(), event.getPlayer().getName(), event.getPlayer().getDisplayName());
			}
		}
	...
	public void onLeave(MarriedPlayerQuitEvent event)
	{
		...
		for(MarriagePlayer partner : player.getPartners())
		{
			if(partner.isOnline())
			{
				messageNowOffline.send(partner.getPlayer().getPlayer(), event.getPlayer().getName(), event.getPlayer().getDisplayName());
			}
		}
	}
	...
	private class MultiPartnerRunnable implements Runnable
	{
		...
		public void run()
		{
			if(player.isOnline())
			{
				...
				{
					messageAllOffline.send(player.getPlayer().getPlayer());
				}
				else
				{
