...
public class Effect_1_9 extends EffectBase
{
	private static final Class<?> EParticle = Reflection.getNMSClass("EnumParticle");
	private static final Class<?> PacketPlayOutParticle = Reflection.getNMSClass("PacketPlayOutWorldParticles");
	...
	public void SpawnParticle(Location loc, Effects type, double visrange, int count, float offsetX, float offsetY, float offsetZ, float speed) throws Exception
	{
		...
		{
			System.out.println("Unable to spawn particle " + type.getName() + ". (Version 1.9)");
			e.printStackTrace();
		}
	}
}