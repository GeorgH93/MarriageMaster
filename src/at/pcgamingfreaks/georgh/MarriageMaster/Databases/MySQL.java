...
public class MySQL 
{
	...
	private void CheckDB()
	{
		try
		{
			...
			stmt.execute("CREATE TABLE IF NOT EXISTS marry_players (`player_id` INT NOT NULL AUTO_INCREMENT, `name` VARCHAR(20) NULL , PRIMARY KEY (`player_id`));");
	...
	public TreeMap<String, String> GetAllMarriedPlayers()
	{
		...
		try
		{
			ResultSet rs = con.createStatement().executeQuery("SELECT player1,player2 FROM marry_partners");
			while(rs.next())
			{
				MarryMap_out.put(rs.getString(0), rs.getString(1));
			}
			rs.close();
		}
		catch (Exception e)
	...
	public Location GetMarriedHome(String playername)
	{
		...
		try
		{
			...
			if(rs.next())
			{
				loc = new Location(marriageMaster.getServer().getWorld(rs.getString(3)), rs.getDouble(0), rs.getDouble(1), rs.getDouble(2));
			}
			rs.close();
			pstmt.close();
		}
	...
	public void SaveMarriedPlayer(String playername, String otherPlayer, String priest)
	{
		...
		try
		{
			int pid = GetPlayerID(playername), opid = GetPlayerID(otherPlayer), prid = GetPlayerID(priest);
	...
	public void SaveMarriedPlayerDivorce(String playername)
	{
		...
		{
			int pid = GetPlayerID(playername);
			PreparedStatement pstmt = con.prepareStatement("DELETE FROM marry_partners WHERE player1=? OR player2=?");
			pstmt.setInt(1, pid);
			pstmt.setInt(2, pid);
			pstmt.execute();
			pstmt.close();
