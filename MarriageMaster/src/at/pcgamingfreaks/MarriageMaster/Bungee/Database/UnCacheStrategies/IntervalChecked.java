...
public class IntervalChecked extends UnCacheStrategie implements Runnable
{
	...
	private final Set<MarriagePlayer> lastOnline = ConcurrentHashMap.newKeySet();
	...
	public void run()
	{
		...
		{
			if(!player.isMarried() && !player.isOnline())
			{
				if(lastOnline.contains(player))
				{
